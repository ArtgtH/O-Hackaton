...
const InputPassword = ({ prev, next, info, isLoading }: Props) => {
  const { data, isPending } = useCheckUserExist(info.email);
  const {
    mutateAsync,
  ...
  const handleSubmit = async (data: LoginDto) => {
    if (!result || isError) await mutateAsync(data);
    next(data, true);
  };
  const isExist = (result && !isError) ?? false;
  return (
    <Formik
      ...
      {({ values, touched, errors }) => (
        <Form className="flex flex-col gap-4 w-full">
          <Typography variant="h3">
            {isExist ? "Введите пароль ниже" : "Вы у нас впервые!"}
          </Typography>
          <TextInput
            type="password"
            name="password"
            ...
            label={`${isExist ? "Пароль" : "Придумайте пароль"}`}
          />
